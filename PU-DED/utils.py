...
def get_matrix(triples1,triples2,entity,rel):
        ...
        # print("2222222222222")
        import sys
        memory = sys.getsizeof(rel_out)
        print("memory")
        print(memory)
        # rel_in = np.concatenate([rel_in,rel_out],axis=1)#将出度和入度进行拼接
        print("//////////////")
        ...
        idx_limit = len(triples)
        origin_index = [i for i in range(idx_limit)]
        ...
        # rel_features = np.concatenate([rel_in,rel_out],axis=1)#将出度和入度进行拼接
        rel_features = rel_out
        adj_features = normalize_adj(adj_features)#对称归一化
        rel_features = normalize_adj(sp.lil_matrix(rel_features))#对称归一化
...
def load_data(lang,train_ratio = 0.3):             
    ...
    y_true=[[0,1]]*node_size #source表示为[1,0]，target表示为[0,1]
    for i in train_pair+dev_pair:
        source_node=i[0]
        target_node=i[1]
        y_true[source_node]=[1,0]
        y_true[target_node]=[1,0]

    target_dis=[]
    for i in train_pair:
        target_dis.append(i[0])
        target_dis.append(i[1])
    
    for i in dev_pair:
        target_dis.append(i[0])
        
    return np.array(train_pair),np.array(dev_pair),adj_matrix,np.array(r_index),np.array(r_val),adj_features,rel_features,target_dis,new_indexes,y_true,max_val1,max_val2
